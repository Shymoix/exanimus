            ...
            while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
                ...
                if (!opponent.bot || (opponent.bot && userTurn)) {
                    await message.channel.send(stripIndents`
                        ${user}, ne yapmak istersin? \`saldır\`, \`savun\`, \`ultra güç\`, \`efsane\` ,veya \`kaç\`?
                        **${message.author.username}**: ${userHP} :heartpulse:
                        **${opponent.username}**: ${oppoHP} :heartpulse:
                    `);
                    const filter = res =>
                        res.author.id === user.id && ['saldır', 'savun', 'ultra güç', 'efsane', 'kaç'].includes(res.content.toLowerCase());
                    ...
                    const choices = ['saldır', 'savun', 'ultra güç', 'efsane'];
                ...
                if (choice === 'saldır') {
                    ...
                    forfeit();
                      ...
                      if(message.author.id === '292341554343837696'){
                        const damage = randomRange(100, guard ? 150 : 500);
                         await message.channel.send(`${user}, Efffffsane atak. **${damage}** hasar vurdun!!`);
                        dealDamage(damage);
                      }else{
                        await message.channel.send('Başarısızsın bro.')
                      }
                      reset();
                } else {
